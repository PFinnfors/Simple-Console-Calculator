...
namespace CalculatorSpace
{
    class Calculator
    {
                ...
                if (menuNum > '0' && menuNum < '7')
                {
                    ...
                    switch (menuNum)
                    {
                        ...
                        case '6':
                            ...
                            breaks the menu loop if yes by setting n to 0, then breaking from the case */
        ...
        static void AddMethod(float addNumber1, float addNumber2, float resultNumber, char charToFloat)
        {
            //Asks for first number, checks if it's in allowed format and gives error message if not
            WriteColor("Green", "\nADDITION:\n");
            Console.WriteLine("Type a number ...");
            charToFloat = Console.ReadKey(true).KeyChar;
            //tryParseResult = float.TryParse(stringToFloat, NumberStyles.Float, CultureInfo.InvariantCulture, out addNumber1);
            ...
            {
                //If in allowed format, does same for second number with another format check
                Console.WriteLine($"Type a number to add to {addNumber1} ...");
                charToFloat = Console.ReadLine();
                tryParseResult = float.TryParse(charToFloat, NumberStyles.Float, CultureInfo.InvariantCulture, out addNumber2);
        ...
        static void SubMethod(float subNumber1, float subNumber2, float resultNumber, string stringToFloat)
        {
            //Asks for first number, checks if it's in allowed format and gives error message if not
            WriteColor("Green", "\nSUBTRACTION:\n");
            Console.WriteLine("Type a number ...");
            stringToFloat = Console.ReadLine();
            tryParseResult = float.TryParse(stringToFloat, NumberStyles.Float, CultureInfo.InvariantCulture, out subNumber1);
            ...
            else //If in allowed format, does same for second number with another format check
            {
                ...
                stringToFloat = Console.ReadLine();
                tryParseResult = float.TryParse(stringToFloat, NumberStyles.Float, CultureInfo.InvariantCulture, out subNumber2);
